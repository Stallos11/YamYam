<template>
    ...
    <div v-else class="grix xs2 gutter-xs7 p-5">
        <div class="p-5" v-if="recipeStore.selectedRecipe?.id">
            <div class="grix xs2 vcenter">
                <p class="font-s4 m-0">{{ recipeStore.selectedRecipe?.name }}</p>
                ...
                <div v-for="ingredient in recipeStore.selectedRecipe?.ingredients" class="d-flex fx-row mb-5">
                    ...
                    <div class="pl-3">
                        ...
                        <span class="text-grey">
                            x{{ (ingredient.amount as number * totalPersons /
                                (recipeStore.selectedRecipe?.eaters_amount as number)).toFixed(0) }}
            ...
            <RecipeComment v-for="comment in commentStore.comments" :comment="comment" />
        </div>
        <div class="p-5">
            <div class="d-flex vcenter">
                <div class="fx-grow bd-2 bd-grey bd-dark-3 bd-b-solid"></div>
...
const totalNutriments = computed(() => {
    ...
    nutriments.value.forEach(nutri => {
        result[nutri] = recipeStore.selectedRecipe?.ingredients.reduce((acc: any, val: any) => {
            //@ts-ignore
