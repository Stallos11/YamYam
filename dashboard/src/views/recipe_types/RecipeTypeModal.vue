<template>
  <RecipeTypeDeleteModal />
  <ax-modal
    class="bg-dark rounded-1 shadow-1 h100"
    v-model="recipeTypeStore.isModalOpened"
  >
    <template #header>
      <div class="grix x1 sm2 vcenter">
        <div class="">
          <p class="my-0 text-primary">
            {{ recipeTypeStore.selectedRecipeType.type }}
            <Icon
              @click="recipeTypeStore.copyRecipeTypeKey('firstname')"
        ...
        <div class="ml-auto font-s5">
          <Icon
            @click="recipeTypeStore.redirEdit()"
            class="ml-auto mr-2 cursor-pointer"
            icon="ri:ball-pen-fill"
            size="100"
          ></Icon>
          <Icon
            @click="recipeTypeStore.showDeleteModal()"
...
import RecipeTypeDeleteModal from "./RecipeTypeDeleteModal.vue";
